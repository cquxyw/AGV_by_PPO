...
def env(ppo,a,s):
    ...
    # s_ = np.array([data.me_x,data.me_y,data.me_yaw,data.me_v,data.me_w])
    s_ = np.array([data.me_x,data.me_y])

    dis_t = (s[0]-goal_x)**2+(s[1]-goal_y)**2
    ...
    reward = - dis_t1 * 0.01 + dis_diff * dis_w
...
def init():   
    data = rospy.wait_for_message('RLin',RL_input_msgs)
    # s = np.array([data.me_x,data.me_y,data.me_yaw,data.me_v,data.me_w])
    s = np.array([data.me_x,data.me_y])
    return s
...
if __name__ == '__main__':
    ...
    for ep in range(EP_MAX):
        ...
        for t in range(EP_LEN):
            ...
            elif (s[0]-goal_x)**2+(s[1]-goal_y)**2 < 0.1:
                v_s_ = ppo.get_v(s_)
                discounted_r = []
                for r in buffer_r[::-1]:
                    v_s_ = r + GAMMA * v_s_
