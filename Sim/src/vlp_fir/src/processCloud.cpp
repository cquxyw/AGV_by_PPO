...
ProcessPointClouds<PointT>:: FilterCloud_D(typename pcl::PointCloud<PointT>::Ptr cloud,float filter_leafSize,int neibor_Num,Eigen::Vector4f minPoint,Eigen::Vector4f maxPoint){
    ...
    Car_roof.setMin(Eigen::Vector4f(-0.5,-0.3,-1,1));
    Car_roof.setMax((Eigen::Vector4f(0.5,0.3,1,1)));
...
//        cout<<count<<endl;
        ...
        int y_grid = floor(double(cloud->points[count].y + (row * grid_size)/2) / grid_size);
