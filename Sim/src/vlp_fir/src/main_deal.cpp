...
//     ----------------------sub and scrb defination----------------------
     point_date_sub = n.subscribe("livox/lidar",10,&object3dDetector::callback,this);
 ...
 void callback(const sensor_msgs::PointCloud2::ConstPtr& msg)
 {
     ...
     pcl::PointCloud<pcl::PointXYZ>::Ptr filterCloud = PointProcessorI->FilterCloud_D(temp_cloud,0.2,50,Eigen::Vector4f(-6,-6,-2,1),Eigen::Vector4f(6,6,5,1));
...
//     --------------------------------Do EUCLEANCluster and segementate plane------------------------------
     ...
     #if 1
     pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_grid = PointProcessorI->Filter_Grid(filterCloud,10,-10,40,60,1);
...
//get segment plane and without
     std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr,pcl::PointCloud<pcl::PointXYZ>::Ptr> segmentCloud = PointProcessorI->segmentPlane_RANSAC(filterCloud,1000,0.1);
